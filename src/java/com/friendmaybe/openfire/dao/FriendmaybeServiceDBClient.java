...
import com.friendmaybe.openfire.logic.FriendmaybeServiceClient;
import com.friendmaybe.openfire.logic.FriendmaybeUser;
import com.friendmaybe.openfire.utils.ApplicationProperties;

public class FriendmaybeServiceDBClient extends AbstractDB implements FriendmaybeServiceClient {
    private static final Logger log = Logger.getLogger(FriendmaybeServiceDBClient.class);
    ...
    private FriendmaybeAnonymousClient friendmaybeAnonymousClient = new FriendmaybeAnonymousClient();
    
    public FriendmaybeServiceDBClient() {
        USER_CORDINATES =
                "SELECT X(location), Y(location) FROM "
                        + ApplicationProperties.getProperty("friendmaybe.login_locations_db") + " l JOIN "
                        + ApplicationProperties.getProperty("friendmaybe.users_db") + " u ON u.id=l.login_id "
                        + " WHERE xmpp=? AND resource=? ORDER BY l.created_at DESC";

        // TODO: filter already connected user very recently
        LOCK_STRANGER =
                "UPDATE "
                        + ApplicationProperties.getProperty("friendmaybe.user_connection_statuses_db")
                        ...
                        + ApplicationProperties.getProperty("friendmaybe.login_locations_db")
                        + " l JOIN "
                        + ApplicationProperties.getProperty("friendmaybe.users_db")
        ...
        LOCK_RANDOM_STRANGER =
                "UPDATE " + ApplicationProperties.getProperty("friendmaybe.user_connection_statuses_db")
                        + " su SET su.user_status=?, su.stranger_id=?, su.stranger_jid=?, su.updated_at=? "
        ...
        GET_MATCHED_USER_FOR_STRANGER_JID =
                "SELECT u.id, user_jid, u.name, birthday, gender FROM "
                        + ApplicationProperties.getProperty("friendmaybe.users_db")
                        + " u JOIN "
                        + ApplicationProperties.getProperty("friendmaybe.user_details_db")
    ...
    public FriendmaybeUser getNewStranger(String userJID) {
        ...
        FriendmaybeUser stranger = this.matchUser(userJID);
        if (stranger == null) {
            stranger = friendmaybeAnonymousClient.getNewStranger(userJID);
...
//                    throw new FriendmaybeException(e);
//                }
//            }
        }
        return stranger;
    ...
    private FriendmaybeUser matchUser(String userJID) {
        ...
        FriendmaybeUser stranger = getMatchedUserForStrangerJID(userJID, lockDateTime);
        if (stranger != null) {
            setMatchedStrangerForUser(userJID, stranger);
            FriendmaybeUser user = getMatchedStranger(stranger.getJID());
            if (user != null) {
                try {
                    cache.set(stranger.getJID(), FriendmaybeUser.getJSONString(user));
                    cache.set(user.getJID(), FriendmaybeUser.getJSONString(stranger));
                } catch (Exception e) {
    ...
    private void setMatchedStrangerForUser(String userJID, FriendmaybeUser stranger) {
        ...
        try {
            ...
            throw new FriendmaybeException("Error on saving user_statuses:" + userJID + " , SQL:\n"
                    + prepareStatement, e);
        } finally {
    ...
    private FriendmaybeUser getMatchedUserForStrangerJID(String strangerJID, Timestamp lockDateTime) {
        Connection connection = null;
        PreparedStatement prepareStatement = null;
        try {
            connection = DbConnectionManager.getConnection();
            ...
            if (result.next()) {
                FriendmaybeUser friendmaybeUser = new FriendmaybeUser();
                friendmaybeUser.setId(result.getLong(1));
                friendmaybeUser.setJID(result.getString(2));
                friendmaybeUser.setName(result.getString(3));
                friendmaybeUser.setDOB(result.getString(4));
                friendmaybeUser.setGender(result.getString(5));
                friendmaybeUser.setConnectedWith(strangerJID);
                return friendmaybeUser;
            }
            return null;

        } catch (SQLException e) {
            throw new FriendmaybeException("Error on LOCK USER" + strangerJID + " , SQL:\n"
                    + prepareStatement, e);
        } finally {
            DbConnectionManager.closeConnection(prepareStatement, connection);
        }
    ...
    private Timestamp lockStranger(Point point, String userJID) {
        ...
        try {
            ...
            throw new FriendmaybeException("Error on LOCK USER" + userJID + " , SQL:\n" + prepareStatement, e);
        } finally {
            DbConnectionManager.closeConnection(prepareStatement, connection);
        }
    }
    ...
    private FriendmaybeUser getCachedMatchedStranger(String matchedKey) {
        Object get = cache.get(matchedKey);
        if (get == null) {
            return null;
        }
        return FriendmaybeUser.getUserFromJSON((String) get);
    }
    ...
    private Point getCordinates(String userJID) {
        ...
        try {
            ...
            throw new FriendmaybeException("Error on getting user cordinates:" + userJID + " , SQL:\n" + prepareStatement, e);
        } finally {
            DbConnectionManager.closeConnection(prepareStatement, connection);
        }
    }
    ...
    public FriendmaybeUser getMatchedStranger(String userJID) {
        ...
        try {
            ...
            if (result.next()) {
                ...
                friendmaybeUser.setId(result.getLong(1));
                friendmaybeUser.setJID(result.getString(2));
                friendmaybeUser.setConnectedWith(userJID);
                friendmaybeUser.setName(result.getString(3));
                friendmaybeUser.setDOB(result.getString(4));
                friendmaybeUser.setGender(result.getString(5));
            ...
            throw new FriendmaybeException("Error on GET_MATCHED_STRANGER_FOR_USER_JID key:" + userJID + " , SQL:\n"
                    + prepareStatement, e);
        } finally {
            DbConnectionManager.closeConnection(prepareStatement, connection);
        }
    ...
    public void clearMapping(String userJID) {
        ...
        try {
            ...
            throw new FriendmaybeException("Error on DELETE user_statuses:" + userJID + " , SQL:\n"
                    + prepareStatement, e);
        } finally {
            DbConnectionManager.closeConnection(prepareStatement, connection);
        }
